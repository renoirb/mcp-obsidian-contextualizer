...
export const vaultIndexerToolDefs = [
    {
        ...
        description: "Returns the current vault-indexer status: counts of indexed files, chunks, and edges.",
        inputSchema: {
            type: "object",
            properties: {},
        },
    ...
    {
        name: "vault_search",
        description: "Search the indexed vault by basename, tags, or full-text. Returns matching file paths with metadata.",
        inputSchema: {
            type: "object",
            properties: {
                q: {
    ...
    },
    {
        ...
        description: "Full metadata for a single indexed file: tags, outgoing links, backlinks, headings.",
        inputSchema: {
            type: "object",
            properties: {
                basename: {
    ...
    {
        name: "vault_orphans",
        description: "Lists notes with zero incoming wikilinks — candidates for cleanup or linking.",
        inputSchema: {
            type: "object",
            properties: {},
        },
    },
    {
        ...
        description: "Lists wikilinks that point to files that don't exist in the vault.",
        inputSchema: {
            type: "object",
            properties: {},
        },
    },
    {
        ...
        description: "Lists one-directional links: A links to B, but B doesn't link back to A.",
        inputSchema: {
            type: "object",
            properties: {},
        },
    ...
    {
        name: "vault_tags",
        description: "Returns tag usage counts across the indexed vault, optionally filtered by prefix.",
        inputSchema: {
            type: "object",
            properties: {
                prefix: {
    ...
    },
    {
        ...
        description: "Triggers a full rebuild of the vault index. This runs the indexer CLI and may take a while for large vaults.",
        inputSchema: {
            type: "object",
            properties: {},
        },
...
export const vaultIndexerToolNames = new Set(vaultIndexerToolDefs.map((t) => t.name));
/**
 * Handle a vault-indexer tool call. Returns the MCP response content.
 * Throws if the tool name is not recognized.
 */
export async function handleVaultIndexerTool(name, args, client) {
    try {
        let result;
        switch (name) {
            ...
            case "vault_file_info":
                result = await client.fileInfo(args.basename);
                break;
            case "vault_orphans":
            ...
            case "vault_missing_backlinks":
                ...
                break;
            case "vault_tags":
                result = await client.tags(args.prefix);
                break;
